...
- Module 'app' uses the bundled annotation processors (moshi, room). CodeAssist runs those itself, so no KSP plugin or processor version exists to export. Apply the KSP plugin and declare each processor to build them with Gradle.
